...
def format_signal_message(symbol: str, result: dict) -> str:
    ...
    tf_line = ", ".join(f"{tf}:{bias}" for tf, bias in result["tf_biases"].items())
    lines = [
        f"{emoji} *{symbol}* — *{result['signal']}*",
        f"Confidence: {result['confidence']}/3 entry checks (entry TF: {result['entry_timeframe']})",
        f"Price: `{result['price']:.5f}`",
        f"Timeframe stack: {tf_line}",
        "",
        "*Reasons:*",
    ]
...
async def generate_and_send_signal(symbol: str, chat_send):
    ...
    try:
        stack = config.MTF_STACK_FOREX if dfetch.is_forex_symbol(symbol) else config.MTF_STACK_CRYPTO

        dfs = dfetch.fetch_multi_timeframe(symbol, stack)
        result = strategy.analyze_mtf(dfs, stack, symbol=symbol)
...
async def backtest_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        trades = await asyncio.to_thread(
            backtest.run_backtest, symbol, stack, limit, 100
        )
